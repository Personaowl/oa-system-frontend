<template>
  <div class="content-grid ai-admin-page">
    ...
    <el-dialog v-model="detailVisible" :title="detail?.docTitle || '文档详情'" width="680px"><el-descriptions v-if="detail" :column="2" border><el-descriptions-item label="知识域">{{ domainLabel(detail.docDomain) }}</el-descriptions-item><el-descriptions-item label="版本">{{ detail.docVersion }}</el-descriptions-item><el-descriptions-item label="状态"><el-tag :type="statusType(detail.status)">{{ statusLabel(detail.status) }}</el-tag></el-descriptions-item><el-descriptions-item label="文件名">{{ detail.fileName || '-' }}</el-descriptions-item><el-descriptions-item label="生效日期">{{ formatDate(detail.effectiveDate, false) }}</el-descriptions-item><el-descriptions-item label="创建时间">{{ formatDate(detail.createdAt) }}</el-descriptions-item></el-descriptions><div v-if="detail?.chunks?.length" class="doc-chunks"><h4>已切分内容</h4><div v-for="chunk in detail.chunks" :key="chunk.id" class="doc-chunk"><strong>{{ chunk.chunkNo }}. {{ chunk.chunkTitle || '内容片段' }}</strong><p>{{ chunk.chunkText }}</p></div></div></el-dialog>
  </div>
</template>
...
const uploadForm = reactive({ file: null, docTitle: '', docDomain: 'ATTENDANCE', docVersion: 'v1.0', effectiveDate: '' })
const editForm = reactive({ id: null, docTitle: '', docDomain: '', docVersion: '', effectiveDate: '' })
const uploadRules = { file: [{ required: true, message: '请选择制度文件', trigger: 'change' }], docTitle: [{ required: true, message: '请输入文档标题', trigger: 'blur' }], docDomain: [{ required: true, message: '请选择知识域', trigger: 'change' }], docVersion: [{ required: true, message: '请输入版本号', trigger: 'blur' }] }
...
function openUpload() { Object.assign(uploadForm, { file: null, docTitle: '', docDomain: 'ATTENDANCE', docVersion: 'v1.0', effectiveDate: '' }); uploadVisible.value = true }
